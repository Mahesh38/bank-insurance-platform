...
| ASM-016 | A bank-approved remote Terraform state backend exists, supporting encryption, locking, versioning and recovery | CR-014, M1.6, M3.3 | Backend named and access granted | **2026-09-19** | **OPEN — but a viable option is now confirmed** | GitLab-managed Terraform/OpenTofu state is available in **Free/CE on self-managed**, so the instance can host state natively; it is encrypted at rest and supports locking. That is an *option*, not the bank standard. `backend.tf` still cannot be written until the bank names the standard — and `C-SEC-6` requires bootstrap state isolated from application state, which a single GitLab-hosted backend must be shown to satisfy |
| ASM-017 | Container and Package Registry are available on the bank instance for the projects that need them | CR-014, M1.7, M6.7 | Feature availability confirmed | **2026-09-19** | **PARTIALLY VALIDATED** | **Container Registry confirmed available 2026-08-29** — the §9.1 image-naming and digest-promotion model stands. **Package Registry was not addressed** and is still open; `contracts` and the shared libraries are the consumers that need it |
| ASM-018 | AWS account and role conventions for OIDC/STS are published, and a bank AWS team can create the identity provider and roles | CR-014, M1.8, M8.3 | Conventions supplied, owner named | **2026-09-19** | **PARTIALLY VALIDATED** | **An AWS account will exist; conventions are explicitly not yet confirmed (bank, 2026-08-29).** The half that matters for M8.1 — account IDs and role naming — is the unconfirmed half: a trust policy is written against real account IDs. M8 stops at design. Static keys remain no fallback (`C-SEC-7`) |
...
| ASM-021 | The bank's GitLab platform or architecture authority will rule on the Appendix C exception for `governance/platform-governance` | CR-014 §6, `ADR-020`, `C-ARC-2` | Written acceptance or refusal | **2026-09-19** | OPEN | Fallback recorded in `DEC-20260829-01` §3.2 — the tree goes to `product/backend` under `governance/`, accepting the `S08-G10` and build-on-every-change costs |
| ASM-022 | The bank GitLab instance **and its storage** are in AWS India regions, or otherwise permissible under the residency standing constraint | CR-014, RISK-021, `C-CMP-1` | Written confirmation of physical residency, ruled by Board 6 | **2026-09-19** | **OPEN — encouraging indicator, not evidence** | `https://gitlab-ce.au.bank.in/` is a self-managed instance on a bank `.in` domain, which makes in-region hosting *likely*. **A domain tells you about DNS, not about where the disks are**, and Shailja rules on storage, backups, CI logs and artifacts — not on a hostname. `C-CMP-1` still blocks the first push and the destination can still be invalid |
| ASM-023 | ~~The bank will confirm the GitLab base URL, version and edition/licence in time for `GLM-001` M3.4~~ | CR-014, RISK-017, M3.4 | Answered by the bank 2026-08-29 | — | **INVALIDATED 2026-08-29** | Remapped from `ASM-012` on merge with `main` (`#83` had already allocated `ASM-012` to EDGE FortiGate coexistence). **Answered, and the answer is worse than the assumption contemplated.** `https://gitlab-ce.au.bank.in/` · **GitLab Community Edition v19.1.2**. The pre-computed consequence assumed Premium; CE is *below* Premium. Required MR approval rules, CODEOWNERS approval enforcement and protected environments do not exist at all. `RISK-017` has **FIRED**; five approved `CR-014` conditions are unsatisfiable as written; raised as `CR-016` |
| ASM-024 | The existing `insurance` parent group exists, its ID is obtainable, and this team is granted subgroup/project creation rights beneath it | CR-014, M1.3, M4.1 | Group ID supplied and a test subgroup created | **2026-09-19** | **PARTIALLY VALIDATED** | Remapped from `ASM-013` on merge with `main` (`#83` had already allocated `ASM-013` to Apigee). **Group confirmed: `https://gitlab-ce.au.bank.in/insurance`, id `820`** — `IMP-11` #3 now has a concrete import target. **Creation rights remain unconfirmed and are the half that gates M4.2/M4.3.** A group ID you cannot create beneath is a read-only fact |
