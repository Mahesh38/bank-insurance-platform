...
| ADR-020 | GitLab CE H0: keep `Insurance/{platform/nip-governance, frontend/nip-app, backend/nip-backend}`; backend stays one Gradle monorepo; no GitHub↔GitLab integration; per-service GitLab projects parked | Proposed (`A3_JOINT_REVIEW`) | Creating one GitLab project per service/lib at H0; GitHub importer/mirroring; moving AIGEM out of `nip-backend` at cutover |
