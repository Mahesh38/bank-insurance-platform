...
| ADR-009 | Hybrid bank connectivity: **attach** workload VPCs as spokes to the existing `AU-CTO-NETWORK` Transit Gateway (do not provision a second TGW); per-environment route tables; Site-to-Site VPN from day one; Direct Connect via the **existing** DX Gateway. `uat` and `prod` may not use CBS or Bank AD stubs | Proposed (`A3_JOINT_REVIEW`) · topology amended 2026-08-31 | Any journey evidenced against a stub outside `dev`; any VPC-to-VPC peering; a second TGW or a second DX circuit "for insurance"; any bank prefix advertised into more than one environment's route table |
| ADR-010 | All egress and inter-VPC traffic is inspected by AWS Network Firewall in a per-environment inspection VPC. **1SB allowlists Apigee IPs, not these NAT EIPs** (`ADR-020`) | Proposed (`A3_JOINT_REVIEW`) · allowlist amended by ADR-020 | Any workload route table with a default route that is not the Transit Gateway; any NAT gateway in a workload VPC; publishing spoke NAT EIPs to 1SB; TLS inspection of a mutually authenticated provider session |
| ADR-018 | North-south **ingress** is Cloudflare Enterprise (SaaS) → F5-XC (SaaS) → Amazon API Gateway → Internal ALB. No External / public ALB. Cloudflare and F5 are not in any platform VPC. **Outbound Apigee is ADR-020** (clause 7 amended) | Proposed (`A3_JOINT_REVIEW`) · clause 7 amended by ADR-020 | Re-introducing a public ALB; drawing Cloudflare or F5 inside AWS / a VPC; putting Apigee on the RM/mobile front door |
| ADR-019 | Backend microservice skeleton scaffold (Gradle modules / repo boundaries for GitLab migration) — not feature code | Accepted (architecture stakeholder, 2026-09-02) | Inventing a new service repo that is not a catalogue context; putting feature code in a skeleton PR |
| ADR-020 | Split API plane: inbound Amazon API Gateway (`ADR-018`); outbound Apigee; 1SB never called from EKS; Apigee IPs are the 1SB allowlist; internal Apigee targets stay private; Dev-inside-UAT account (two VPCs); no CUG at R0; AD-verify via bank API, partners in IdP, Fireframe/NIP-APP UI | Proposed (`A3_JOINT_REVIEW`) | Calling 1SB origin from EKS; publishing spoke NAT EIPs to 1SB; hairpinning internal APIs through Cloudflare/F5; a separate Control Tower `dev` or CUG account at R0; LDAP from EKS to AD; Keycloak chrome as the bank UI |
| ADR-021 | NIP-APP screen descriptor: closed `FORM`/`LIST`/`CARD`/`CAROUSEL` surfaces; closed widget enum; nested `reveals` (max depth 3) + `visibleWhen`; `SCREEN_DOCUMENT`/`SCREEN_ACTION` in Configuration #19; L1 `FormRuntime` + L2 owning invariants; submit `actionId` → closed command; capture on `screen_submission`; no 1SB envelope; no raw S3 | Proposed (`A3_JOINT_REVIEW`) · runtime amended 2026-09-24 | A second form DSL or 1SB schema on Flutter; unbounded widget plugins; raw S3 `iconUrl`; treating login/pipeline/payment-status as FORM; a form microservice that owns Lead state; client-invented POST URLs |
...
| ADR-016 | Enterprise perimeter, integration and delivery baseline: Cloudflare (Edge CDN/DDoS), F5-XC / WAF (Bank Policy, SaaS), EBS (Enterprise Service Bus) for Core Banking (CBS / CIF), GitLab CI/CD, Terraform IaC, CloudTrail + CloudWatch. **Ingress hop amended by ADR-018** — External ALB before API Gateway is withdrawn | Proposed (`A3_JOINT_REVIEW`) · hop 1 amended by ADR-018 | Bypassing bank enterprise perimeter; placing Cloudflare or F5 in AWS / a platform VPC; a public ALB in front of API Gateway; direct database or unmanaged point-to-point connections to CBS; omitting CloudTrail management auditability; manual console drifts |
...
| GOV-009 | 2026-09-23 | **AIGEM 1.5 flow**: SF5 PARALLEL lanes (LC-2); evidence-based unpark (BR-5); all-criteria-MET → CANDIDATE (SG-2); evidenced-blocker T4 relief (RG-9); DEP-4 soft-default; GATE-S08 marked CANDIDATE | Over-serialization parked dependency-safe work behind stage names and held unpark keys behind human PASS while S08 was already 10/10 MET | **CANDIDATE transcribed under ADMIT-BYPASS** — human Architecture + Product (+ Security/Compliance on RG-9) ratification outstanding; see [CR-016](../change-requests/CR-016-parallel-lanes-evidence-unpark.md) |
...
| CR-014 | 2026-09-03 | SCOPE | Stakeholder pull: WS-1 Life LOB adapter (Term+Savings+ULIP), bank-model extraction, typed 1SB JSON, packaging standards, poll/retry + circuit breaker — `EPIC-002` | **CANDIDATE** — transcribed into WS-1 scope artefacts | Human T4 Architecture / Security / Compliance / Operations outstanding. WS-3 R0 catalogue/journey for Savings/ULIP is CR-015. [`CR-014`](../change-requests/CR-014-ws1-life-lob-adapter-standards.md) |
| CR-015 | 2026-09-12 | SCOPE | Stakeholder: complete R0 assisted Life journey (Term + Saving/ULIP e2e) — `EPIC-004` | **CANDIDATE** — transcribed into WS-3 scope artefacts under ADMIT-BYPASS (HALT lifted) | Rajal (R1) + Mahesh (R2) HUMAN verdicts outstanding. Listing them as approvers is not a signature. [`CR-015`](../change-requests/CR-015-ws3-r0-savings-ulip-journey.md) · origin `SUG-20260911-uls` |
| CR-016 | 2026-09-23 | GOV | Parallel lanes (SF5), evidence-based unpark, SG-2 CANDIDATE, RG-9 evidenced-blocker T4 relief, DEP-4 — AIGEM 1.5 | **CANDIDATE** — L1 files transcribed under ADMIT-BYPASS | Architecture + Product (+ Security/Compliance on RG-9) human ratification outstanding. [`CR-016`](../change-requests/CR-016-parallel-lanes-evidence-unpark.md) · origin `SUG-20260923-par` |
