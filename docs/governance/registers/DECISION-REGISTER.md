...
| — | Persistence is platform-common (`bank-persistence-service`), reached over HTTP | **Superseded by `ADR-019`** | Do not cite this row. The service is Integration Ops / Evidence, not a platform DB gateway |
| — | Integration service owns no Flyway/JPA | Accepted | Any persistence change in the integration service. **Stands** — HTTP to the Integration Ops store; other contexts own their own Flyway (`ADR-019`) |
...
| ADR-019 | Persistence ownership is per bounded context; `bank-persistence-service` is Integration Ops / Evidence, not a platform-wide HTTP DB gateway; Flyway-only mega-migrator rejected | Proposed (`A3_JOINT_REVIEW`) · `CR-015` | Any proposal to persist a business context through `bank-persistence-service`; any Flyway-only bag of all scripts; any cross-schema grant; any JDBC from a consumer into persistence-owned tables |
...
| DB-DEC-0003 | `bank-persistence-service` remains Integration Ops / Evidence; do not extend to business contexts; Flyway-only mega-migrator rejected | AI-DRAFTED (`APPROVED_WITH_OBSERVATIONS`, `D1`) — human Aarti signature outstanding | Extending V1 with Lead/Consent/Payment-context/Policy; a Flyway-only bag of all scripts; JDBC from consumers into persistence-owned tables |

Full records: [`DB-DEC-0001`](../../platform/data-architecture/DB-DEC-0001-r0-physical-model.md) · [`DB-DEC-0002`](../../platform/data-architecture/DB-DEC-0002-cr013-alignment-review.md) · [`DB-DEC-0003`](../change-requests/CR-015-persistence-ownership-per-context.md) (inlined in `CR-015` §6).
...
| CR-015 | 2026-09-04 | CONSTRAINT | Narrow `bank-persistence-service` to Integration Ops / Evidence; persist per bounded context — `ADR-019`. `CR-014` skipped: in flight on PR #86 (Life LOB) and PR #82 (GitLab) | **CANDIDATE** — transcribed into ADR + persistence contract | Human T4 Architecture / Database outstanding. Board drafts in [`CR-015`](../change-requests/CR-015-persistence-ownership-per-context.md) |
