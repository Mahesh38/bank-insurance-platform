...
| ADR-019 | Backend microservice skeleton scaffold in the Gradle monorepo; catalogue maps context → module → port → logical GitLab group; GitLab project-per-module split deferred | Accepted (stakeholder, pre-GitLab) | Feature code in skeletons; treating catalogue `gitlab_group` as a project-creation script at S08 (see ADR-020) |
| ADR-020 | GitLab CE H0: keep `Insurance/{platform/nip-governance, frontend/nip-app, backend/nip-backend}`; backend stays one Gradle monorepo; Wave 1 copy is all libs + 1SB + persistence (`SUG-20260910-w1s`); later services copy into the same project when mature; no GitHub↔GitLab integration; per-service GitLab projects parked | Proposed (`A3_JOINT_REVIEW`) | Creating one GitLab project per service/lib at H0; copying 1SB without `bank-persistence-service`; GitHub importer/mirroring; moving AIGEM out of `nip-backend` at cutover |
...
| CR-014 | 2026-09-03 | SCOPE | Stakeholder pull: WS-1 Life LOB adapter (Term+Savings+ULIP), bank-model extraction, typed 1SB JSON, packaging standards, poll/retry + circuit breaker — `EPIC-002` | **CANDIDATE** — transcribed into WS-1 scope artefacts | Human T4 Architecture / Security / Compliance / Operations outstanding. WS-3 R0 catalogue/journey for Savings/ULIP **not** admitted. [`CR-014`](../change-requests/CR-014-ws1-life-lob-adapter-standards.md) |
