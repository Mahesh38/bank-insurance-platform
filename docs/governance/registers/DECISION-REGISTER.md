...
| ADR-021 | NIP-APP screen descriptor: closed `FORM`/`LIST`/`CARD`/`CAROUSEL` surfaces; closed widget enum; nested `reveals` (max depth 3) + `visibleWhen`; same validation object on client and server; no 1SB envelope; no raw S3 | Proposed (`A3_JOINT_REVIEW`) | A second form DSL or 1SB schema on Flutter; unbounded widget plugins; raw S3 `iconUrl`; treating login/pipeline/payment-status as FORM |
