...
| ADR-002 | Workstream topology: WS-3 is the platform, WS-1 a supplier, WS-2 an enabler | Proposed (`A4_HUMAN_REQUIRED`) · **operating posture amended by ADR-020** (service execution lanes are not programme workstreams) | Any proposal to grow WS-1 into the platform or register a fourth *programme* workstream for the client; Flutter remains inside the programme surface |
| ADR-019 | Backend microservice skeleton scaffold for catalogue modules and GitLab group mapping | Accepted (Mahesh, 2026-09-02) | Inventing ad-hoc module layouts outside the scaffold/catalogue |
| ADR-020 | Two-tier workstreams: continuous North Star programme (`WS-NS`) + one service workstream (`SWS-*`) per microservice with progress boards and dependency sync | Accepted (Architecture — Mahesh, 2026-09-13) · Product/Delivery countersign outstanding for state transcription | Second competing programme workstream; giving `SWS-*` its own lifecycle `stage_status`; skipping cross-service sync checks |
...
| ADR-018 | North-south ingress is Cloudflare Enterprise (SaaS) → F5 Distributed Cloud / F5-XC (SaaS) → Amazon API Gateway → Internal ALB. No External / public ALB. Cloudflare and F5 are not AWS services and are not placed in any platform VPC. A candidate bank API plane is **not drawn** until `SPIKE-001` returns (`ADR-018` clause 7) | Proposed (`A3_JOINT_REVIEW`) | Re-introducing a public ALB in front of API Gateway; drawing Cloudflare or F5 inside the AWS region box or a VPC; treating F5-XC as an in-VPC BIG-IP appliance; drawing the candidate bank API plane before SPIKE-001 answers |
...
| GOV-009 | 2026-09-13 | Adopt **two-tier workstream operating model**: continuous North Star programme lane (`WS-NS`) + one service workstream per microservice (`SWS-*`) with progress markdown boards and dependency sync checks; amend ADR-002 operating posture via ADR-020 / CR-016 | Multi-agent delivery needs governed ownership per deployable without exploding lifecycle stages | **APPROVED (Architecture) by Mahesh, 2026-09-13**; Rajal (R1) and Kalpana (R12) countersignatures outstanding before `CURRENT-STATE.yaml` topology transcription |
...
| CR-016 | 2026-09-13 | GOV | North Star continuity programme lane + per-microservice service workstreams, progress boards, dependency sync — `ADR-020` | **APPROVED (Architecture)** 2026-09-13 by Mahesh; Product/Delivery countersign + state transcription outstanding | Mahesh (R2) APPROVED. Rajal (R1) + Kalpana (R12) outstanding. [`CR-016`](../change-requests/CR-016-north-star-and-service-workstream-strategy.md) · origin `SUG-20260913-wss` |
