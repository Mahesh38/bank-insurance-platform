...
|----|------|----|-----------|--------------|----------------|------------------|----------------|----------------|
| → [SUG-20260913-fnd](./SUGGESTION-REGISTER.md#sug-20260913-fnd--onesb-demo-missing-documented-fund-routes) | 1SB demo 404 on documented ULIP `POST /insurance/lifesave/v1/fund/list` and `/fund/performance`. Bank adapter already wired (FUNC-027). Do not retarget to `/quote/ulipList`. | WS-1 | Phase 4 | Phase 4 / GATE-P4 4.1 | Demo returns non-404 on those documented paths, or portal publishes a replacement path that demo also routes | MUST | P4 / P1 | Provider deploy/docs mismatch; a bank workaround would invent an API |
