...
|----|------|----|-----------|--------------|----------------|------------------|----------------|----------------|
| → [SUG-20260909-glc](./SUGGESTION-REGISTER.md#sug-20260909-glc--gitlab-ce-backend-monorepo-not-per-service-projects) extract | One GitLab CE project per Gradle module / bounded context (the previous `GITLAB-REPO-STRUCTURE.md` map) | WS-3 | S08 | S09+ | S09 platform foundation green **and** `bank-common-*` published to GitLab Maven Package Registry **and** the extracted service has its own release cadence | SHOULD | P4 / P2 | H0 is `insurance/backend/nip-backend` as one Gradle monorepo (`ADR-020`). **`SUG-20260910-w1s` does not unpark this** — “one service at a time” means later MRs *into* `nip-backend`, not new GitLab projects. CE lacks MR dependencies and required CODEOWNERS. Skeletons are not deployables. GATE-S08-G1 needs one pipeline. |
