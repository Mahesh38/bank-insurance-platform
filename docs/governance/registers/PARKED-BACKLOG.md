...
|----|------|--------|-------------|------------|
| → [SUG-20260825-st2](./SUGGESTION-REGISTER.md#2-register) | NIP-APP (workforce Flutter) on Play Store / Apple Store; customer store apps | 2026-08-25 | Recurrence 2. Same NIP codebase for iOS/Android is `SUG-20260825-nip`. Customer store apps are R1 (`#1`). Public-store listing vs MDM for the workforce client is LLD §14 / S11 (Rajal + Deepali) — trust boundary, not an architecture default | S11 Flutter hosting decision is taken, or DIY is unparked at R1 |
