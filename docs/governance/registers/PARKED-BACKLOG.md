...
| → [SUG-20260827-prp](./SUGGESTION-REGISTER.md#sug-20260827-prp--polyrepo-split-one-github-repository-per-service) | Split the Gradle monorepo into one GitHub repository per microservice and app; parent repo(s) for docs and config; create and cross-link the remotes | WS-3 | S08 | S09 (ADR); implement only after GATE-S08 PASSES | GATE-S08 PASSES, and an accepted polyrepo ADR exists after Boards 1+2+4+7, **or** a second team needs write isolation CODEOWNERS cannot provide | SHOULD | P5 / P3 | Chat instruction is not an ADR. Deployable ≠ git repo. GATE-S08 would reset. Cheaper control: CODEOWNERS. This agent cannot create GitHub repos (read-only token). |
