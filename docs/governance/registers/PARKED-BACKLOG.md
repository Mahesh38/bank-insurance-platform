...
| [ADR-019](../../platform/architecture-review/08-architecture-decision-log.md) allocation | Allocate the `bank_persistence` tables to their owning bounded contexts — per-context schema, credentials, Flyway history and repository layer; `bank-persistence-service` narrows to the Integration Operations/Evidence context | WS-3 | S08 | **S09** | **`CR-014` cutover complete** — GitLab declared authoritative and the freeze released | MUST | P4 / P1 | Target approved by `CR-015` Option B on 2026-08-29. Parked by the same decision: `AC-5` forbids combining persistence restructuring with the repository migration, because a boundary change inside a repository move cannot be audited. Aarti's integrity and recovery review (Q4) is unanswered and is a precondition, not a step |
