...
|----|------|----|-----------|--------------|----------------|------------------|----------------|----------------|
| → [SUG-20260922-1ed](./SUGGESTION-REGISTER.md#sug-20260922-1ed--quotation-data-lifecycle-decision) | Quotation Data Lifecycle decision: Lead → Suitability versions → Quote Batch → Provider Attempts → Offers → Presentation/Evidence snapshot; narrow Aurora rows + S3 payloads; hot/warm/cold; retention classes vs OPEN-I4. No Mongo. Reshape `02` §4.5 + `08-quotation.sql` before first Quotation persist. | WS-3 | S08 | S09 — before Quotation Flyway apply | S09 Quotation schema-apply opens, OR Quotation BC implementation starts, OR Board 6 opens OPEN-I4 for Quote/Offer — whichever first; full re-triage | MUST | P4 / P1 | SF2 absorption fails (new ADR + aggregates). Parts already decided: DB-DEC-0001 (no second engine), S3 bytes, INV-SUI-01, OPEN-I6 no R0 partition. Do not selective-delete until OPEN-I4 |
| → [SUG-20260913-fnd](./SUGGESTION-REGISTER.md#sug-20260913-fnd--1sb-demo-missing-documented-fund-routes) | 1SB demo 404 on documented ULIP `POST /insurance/lifesave/v1/fund/list` and `/fund/performance`. Bank adapter already wired (FUNC-027). Do not retarget to `/quote/ulipList`. | WS-1 | Phase 4 | Phase 4 / GATE-P4 4.1 | Demo returns non-404 on those documented paths, or portal publishes a replacement path that demo also routes | MUST | P4 / P1 | Provider deploy/docs mismatch; a bank workaround would invent an API |
...
| → [SUG-20260831-apg](./SUGGESTION-REGISTER.md#sug-20260831-apg--apigee-is-the-bank-api-plane--do-not-add-a-second-amazon-api-gateway-until-confirmed) remaining | Written Apigee-team answers that `ADR-020` still needs: edition; NIP as a product; **private** spoke→Apigee hostname (no Cloudflare/F5 hairpin); 1SB (and PG outbound) **egress IPs per env**; per-API onboarding; PG **callback** inbound hop confirmation | WS-3 | S08 | S09 P4 | Bank API platform + network write the six answers | MUST | P4 / P1 | **Draw of outbound Apigee UNPARKED 2026-09-14 into `ADR-020`.** Ingress stays Amazon API Gateway. Do not publish spoke NAT EIPs to 1SB. Absence of the six answers does not keep Apigee off the egress picture |
...
| → [SUG-20260913-hms](./SUGGESTION-REGISTER.md#sug-20260913-hms--hub-owned-bank-language-masters) | Persist Integration Hub masters in **bank language**: quote-category as one common Hub list (if `ASM-014` holds); proposal-category insurer-keyed. Translate to BFF/RM. Do not forward 1SB `entityIds`. Not Redis/`TD-010`. | WS-1 | L7 / S08 | S11 / first named Hub master API for BFF | NIP BFF or RM quote/proposal UI needs Hub-owned dropdowns (S11 assisted Life screens / EPIC-004 FUNC), or a named Hub master API for BFF is opened | MUST | P4 / P2 | SF2 absorption fails (new catalog + BFF contract). Standing hop recorded as `SUG-20260913-acl`. Today's `POST /v1/master-data/lookup` stays Hub-internal / provider-shaped |
