...
| RISK-012 | **Audit events are not persisted.** `LoggingAuditEventPublisher` is the only implementation wired into `1sb-integration-service`, so the trail lives in application logs; nothing writes to the `audit_event` table. Immutability, 7-year retention and queryability are therefore claimed but not provided | WS-1 | 3 | 3 | 9 | Tech Lead + Compliance | Raised to Compliance as **Finding 1** of the [4.4 review pack](../../1sb-insurance-integration/service-ssot/compliance/COMPLIANCE-REVIEW-PACK.md); fix is an HTTP adapter against the existing `POST /internal/v1/audit-events`, tracked as SUG-20260813-a1c | **Now** — any UAT exposure to real customer data, or gate 4.4 review |
