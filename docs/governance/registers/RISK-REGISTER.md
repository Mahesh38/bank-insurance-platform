...
| RISK-017 | The bank GitLab edition is Premium rather than Ultimate, so MR-blocking security policies do not exist and `S08-G5`'s mechanism has no direct equivalent | WS-3 | 2 | 3 | 6 | Deepali / Security | Resolve at `GLM-001` M1.2 before M3.4 fixes module capability. Deepali has **declined to pre-approve** a control downgrade she cannot yet see; a Premium answer reopens the mechanism, not the requirement | M3.4 starts with the edition unconfirmed, or anyone proposes "scan and triage manually" as equivalent without a signed exception |
