...
| RISK-016 | ~~A live credential sits in the history~~ → **CONFIRMED 2026-08-29.** Finding B, `RAW_PAYLOAD_ENCRYPTION_KEY`, in the 358-commit / 81-branch history and is pushed into the bank estate, converting a personal-repo exposure into a bank security incident with disclosure obligations | WS-3 | 2 | 3 | 6 | Deepali / Security | `C-SEC-1` blocking full-history scan before the first push; `C-SEC-2` rotate → scrub → migrate, in that order. Never scrub before rotating — it leaves the live secret live and destroys the evidence of what to rotate | Any proposal to push history before the scan is clean, or to scrub before rotating |
...
| RISK-025 | ~~The `pre-gitlab-migration` rollback anchor exists only locally~~ → **ON REMOTE 2026-09-01.** Annotated tag `pre-gitlab-migration` points at `b8027751738b04d00dbe071a77b2aba56828a2cd` and `git ls-remote --tags origin refs/tags/pre-gitlab-migration` is populated. `C-OPS-1` still requires it to remain reachable before the freeze | WS-3 | 1 | 2 | 2 | Shivanshi / SRE | Keep the tag; execute `C-OPS-2` rollback drill before freeze lift | The freeze is scheduled while `git ls-remote --tags origin refs/tags/pre-gitlab-migration` is empty |
| RISK-026 | Finding B — the `RAW_PAYLOAD_ENCRYPTION_KEY` baked into the Dockerfile 2026-08-04 — is dispositioned **COMPROMISED / POTENTIALLY LIVE** (operator ruling 2026-08-29): a runnable Docker default cannot be declared dead without deployment evidence. If any stored `raw_payload` row decrypts with it, regulated PII under 7-year retention is encrypted under a key that was public in `main`'s history | WS-3 | 2 | 3 | 6 | Deepali / Security + Aarti / Database | `C-SEC-2` in full and in order: rotate or formally retire, determine whether any row decrypts with it, re-encrypt or dispose of affected data, **then** import HEAD as an orphan (`CR-017`). Orphaning HEAD does **not** replace rotation. Source-history `filter-repo` remains barred; the source graph is a sealed bundle (`AC-8`) | Any GitLab import before rotation/retirement is recorded, or any claim that the key is dead without deployment evidence |
...
| RISK-029 | The sealed offline bundle of personal-forge history (`AC-8`) is lost, leaked, or later pushed to GitLab, re-introducing the identity `CR-017` removed — or it is destroyed before Finding B rotation evidence is complete | WS-3 | 2 | 3 | 6 | Shivanshi / SRE + Shailja / Compliance | Create the bundle at M5.2 onto media Board 6 names; do not put it on a GitLab remote; dispose only after Finding B is closed and `C-CMP-4` names destroy vs legal-hold | Any `git push` of the bundle, or destruction before `C-CMP-4` |
| RISK-030 | Operators treat the personal GitHub workbench as a git remote of GitLab (`fetch` / pull-mirror / `push --all`), re-importing personal identity and creating two sources of truth — the failure `CR-014` already rejected | WS-3 | 2 | 3 | 6 | Shivanshi / SRE + Amit / Engineering | `AC-7` and [`WORKBENCH.md`](../../platform/gitlab-migration/WORKBENCH.md): tree snapshot only. `identity-guard.py` on every GitLab commit; a Gmail/Anthropic/Cursor author is a failed import, not a style issue | Any GitLab commit whose author/committer/message fails `identity-guard.py`, or a GitLab project with GitHub pull-mirroring enabled |
