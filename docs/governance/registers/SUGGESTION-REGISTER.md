...
| SUG-20260907-fig | 2026-09-07 | human:front-architect | Figma extras on the same journey: ULIP-leads tab, Health product picker, meeting scheduler after create. **Savings/ULIP picker carved 2026-09-12** to EPIC-004 / CR-015 | SF3 | SC2 | MUST | FUNC | P4 / P2 | PARKED | [PARKED-BACKLOG](./PARKED-BACKLOG.md#1-parked--scheduled-work) · [detail](#sug-20260907-fig--figma-ulip--health--meeting-extras) |
...
| SUG-20260821-jx2 | 2026-08-21 | human:Mahesh | Extend the Journey Execution Specification beyond R0 to the whole application — DIY/customer journey, hybrid mode switching, Group B insurers, Health/Motor/Travel, renewals and servicing. Admin UI and Reporting/MIS carved into R0 by CR-013. **Assisted ULIP/Savings JES carved 2026-09-12** to EPIC-004 / CR-015 | SF3 | SC2 | NOT-NOW | DOC | P5 / P2 | PARKED | [PARKED-BACKLOG](./PARKED-BACKLOG.md#1-parked--scheduled-work) · recurrence_count 4 (2026-09-12: assisted ULIP/Savings JES left this parked bag) |
...
action_rationale: >
  ...
  different live path. ALB catch-all is not a documented 1SB API.
...
outcome:
  ...
  plan_id: null
  status: REJECTED
  closed_reason: "ALB catch-all is not a documented 1SB API"
...
priority:
  now: P4
  at_target: P1
  factors: { N: 0, S: 1, B: 0, R: 2, D: 0, E: 1 }
  score: 3
  matrix_default: P4
  consistency: OK
  overrides_applied: []
  caps_applied: []
  rationale: "NOT-NOW provider deploy gap; P1 only when claiming live fund E2E"
...
necessity:
  ...
  failure_without_it: >
    Bank cannot call documented Get Product UI Data or ULIP fund list/performance;
    field guides still described guessed /quote/ulipList paths.
  evidence_tier: E2
  evidence:
...
classification:
  ...
  epic: EPIC-002
  risk_tier: T2
  destination: "PRODUCT-BACKLOG.md FUNC-027"
...
priority:
  ...
  matrix_default: P2
  consistency: OK
  overrides_applied: ["O4"]
  override_evidence:
    - "api-catalog and portal document Product UI Data and ULIP fund list/performance; adapter did not expose them; guessed /quote/ulipList must not be used"
  caps_applied: []
  rationale: "O4 missing documented provider APIs named by the Life catalog"

dependencies:
  edges: []
  state: READY
  enablement_count: 0
  earliest_start: "FUNC-027 this PR"
  cycles: none

breakdown:
  children: []
  completion_definition: >
    Bank GET /v1/products/ui-data and POST /v1/ulip/funds/list|performance call
    only the portal-documented 1SB paths; tests assert those paths and never
    /quote/ulipList
  not_included:
    - "save/send quote (not documented)"
    - "invented lifesave getproductuidata"
...
context:
  ...
  state_as_of: "2026-09-11"
  state_provisional: false
  active_work_item: "GATE-P4 / 4.1 sandbox E2E (WS-1) + admitted EPIC-002 adapter; EPIC-004 docs/contracts"

stage_fit:
  code: SF2
  rationale: >
    HALT lifted (FreshnessCheck 2026-09-12 exit 0). CR-015 CANDIDATE transcribed under
    ADMIT-BYPASS. Docs/JES/contracts are SF2 absorbable-adjacent at S08 (same split as
    EPIC-003). Flutter/BFF sale-path implementation remains SF3 until S11 and is
    PARKED-DEPENDENT on GATE-S08, GAP-006, GAP-007. Recurrence 2026-09-12 widened
    the objective from "Term first, Savings/ULIP extra" to complete assisted Life
    e2e (Term and Saving/ULIP).
  target_stage: "S11 / R0 assisted Life journey — docs/contracts at S08; full sale path at S11"
  unpark_trigger: "CR-015 HUMAN APPROVED by Rajal (R1) + Mahesh (R2) for production use; GATE-S08 PASSED and GAP-006 / GAP-007 closed before S11 screens"
  absorption_test:
    small: false
...
scope:
  code: SC0
  business_scope: >
    R0-SCOPE A1 already names Life = Term, ULIP, Savings. Current increment now
    matches A1 for the assisted journey (CR-015 transcription). DIY/hybrid (A2)
    remain out of this increment.
  serves: ["R0-ASSISTED-LIFE-SALE"]
  failure_without_it: >
    Bank RM cannot complete an assisted Savings or ULIP sale; CR-014 adapter
    readiness has no WS-3 journey consumer.
  minimal: true
  authority: "Stakeholder decision 2026-09-11 restated 2026-09-12 → CR-015"

necessity:
  now: MUST
  future_necessity: MUST
  target_stage: "R0 assisted journey (S11 implementation)"
  binds_when: "Stakeholder requires complete assisted Life (Term and Saving/ULIP) e2e in R0"
  failure_without_it: >
    Bank RM cannot complete an assisted Savings or ULIP sale; CR-014 adapter
    readiness has no WS-3 journey consumer.
  evidence_tier: E2
  evidence:
    ...
    - "Verbatim human restatement 2026-09-12: complete life insurance assisted journey in R0 including term and saving/ulip end2end; CR-015 CANDIDATE"
    ...
    - "FreshnessCheck 2026-09-12 exit 0 (HALT lifted)"
  confidence: C5
  assumptions: []
  anti_over_engineering:
    X1_named_consumer: true
...
action: ADMIT-BYPASS
action_rationale: >
  HALT lifted. Human restated CR-015 as CANDIDATE with required approvers Rajal
  (R1) and Mahesh (R2) and widened intent to complete assisted Life e2e.
  09 §8 ADMIT-BYPASS transcribes CANDIDATE into scope artefacts (CR-013/014
  pattern). Listing approvers is not a HUMAN T4 signature; decided_on stays
  null. 14 §3: do not implement Flutter/BFF/JES code until APPROVED. 08 §5:
  parked rows are split, not deleted. EPIC-004 minted for docs/contracts;
  FUNC-021 screens remain BLOCKED.
bypass_authorised_by: "human:stakeholder 2026-09-12 (complete assisted Life e2e; CANDIDATE; ADMIT-BYPASS transcription after HALT lift)"
duplicate_of: null
conflicts:
  - "Resolution: WS-3 out_of_scope ULIP/Savings row removed; catalogue in_scope expanded; objective R0-ASSISTED-LIFE-SALE"
...
classification:
  ...
  epic: EPIC-004
  risk_tier: T4
  destination: "CR-015 CANDIDATE transcribed · EPIC-004 minted · screens S11"
...
priority:
  ...
  caps_applied: []
  rationale: >
    Stakeholder MUST for complete R0 assisted Life (Term + Saving/ULIP). P2 while
    S08 foundation is open (contracts/JES only). P1 at S11. Not a hard P1
...
dependencies:
  ...
  enablement_count: 0
  earliest_start: "Docs/contracts now (S08); screens after CR-015 HUMAN APPROVED, GATE-S08, GAP-006, GAP-007"
  cycles: none

breakdown:
  children: [DOC-022, ARCH-024, FUNC-020, FUNC-021, QA-013]
  completion_definition: >
    JES + NIP BFF/RM contract deltas + R0 catalogue matrix for Term+Savings+ULIP
    AND (at S11) assisted sale path with Term non-regression.
  not_included:
    ...
    - "Manufacturing HUMAN T4 signatures"

outcome:
  registered_in: "registers/SUGGESTION-REGISTER.md · change-requests/CR-015 · EPIC-004 · PARKED-BACKLOG split note"
  work_item_id: EPIC-004
  plan_id: null
  status: ADMIT-BYPASS
  closed_reason: null

resumed: "GATE-P4 / 4.1 sandbox E2E; EPIC-004 docs/contracts may proceed in parallel"
```

Recurrence 2026-09-12 (CS-2, not a new row): human restated CR-015 as `decision: CANDIDATE` with required approvers Rajal (R1) and Mahesh (R2), and widened the product intent to **complete assisted Life in R0 (Term and Saving/ULIP end to end)**. HALT is lifted. ADMIT-BYPASS transcribed scope into artefacts; HUMAN T4 still outstanding. Listing those names is not a signature.
...
source: "Cloud agent intake — RM app / BFF API contracts for lead landing and create"
input: >
  As front architect we decided to develop the lead phase for the application and
  start building API contract for RM app and BFF, so how RM will search the customer,
  see his own lead, prospects, and till the time he successfully create lead. We need
  to identify the API we need to develop, what will be the flow of API, what will be
  in the request body, what will be in response, how can we make sure we dont send
  all data in bulk and optimise the APIs, add pagination where required, add polling
  where required, do parallel API calls where required, what will be my POJO
  structure, we need all the documents for this lead creating and landing page view
  phase e2e.
...
stage_fit:
  code: SF1
  rationale: >
    FF-15 (consumer-driven contract tests) is assigned to S08. The documentation canon
    records NIP BFF OpenAPI as 1SB-only. R0-HLD §5.1 is an unpaginated sketch that
    still publishes /opportunities after ADR-014. Writing the consumer contract for
    SCR-02..SCR-05 is completing that S08/S07 contract artefact, not implementing the
    RM journey (S11). FreshnessCheck at intake: WARN (state_as_of 28d; 04-STAGE_GATES.md
    stale); admit allowed with disclosure. Exit 1, not 2.

scope:
  code: SC0
  business_scope: >
    Explicit in R0-SCOPE §3 Lead (create) and Customer (CBS fetch by Cust ID / Mobile /
    PAN) and in S11-E02 / S05 SCR-02..SCR-05. Single-RM create is in; campaign/bulk is
    out. Constrained to LIFE/TERM.
  serves: []
  failure_without_it: >
    S11-E02 cannot start contract-first; FF-15 has no NIP BFF consumer contract; HLD
    paths contradict CR-013.
  minimal: true
  authority: "R0-SCOPE.md §3 · S05-experience-evidence.md §4.3 · AC-CUST-010 · AC-LEAD-010-1 · FF-15 · CR-013"

necessity:
  now: SHOULD
  future_necessity: MUST
  target_stage: "S11 — Vertical Slice"
  binds_when: "S11-E02 / S11-E06 implementation of SCR-02..SCR-05"
  failure_without_it: >
    Flutter and BFF would implement against Figma and a drifted /opportunities sketch.
  evidence_tier: E2
  evidence:
    - "R0-HLD.md §5.1 sketch; no pipeline or search resources"
    - "05-DOCUMENTATION-CANON.md S07 API contracts row 1SB only"
    - "03-solution-architecture-r0.md FF-15 assigned to S08"
    - "S05 SCR-02..SCR-05 and AC-CUST-010 / AC-LEAD-010-1 already written"
  confidence: C4
  assumptions: []
  anti_over_engineering:
    X1_named_consumer: true
...
action_rationale: >
  ...
  PRI-8 floor B for SF1 non-MUST is 0; B raised to 2 because this enables S11-E02,
  S11-E06 and FF-15. Matrix default P3; score one band away (PRI-4 OK). Documents
  only; S08 feature-breadth bar is not crossed. Figma extras split to SUG-20260907-fig.
conflicts: []
...
classification:
  ...
  epic: EPIC-003
  risk_tier: T3
  destination: "docs/platform/ws3-platform/EPIC-003.work-item.yaml"
  rationale: >
    New public API contract (T3). G2 considered for PII field set; RG-6 tiers T3
    because this specifies masking, it does not change a runtime control. Security
    and Compliance may escalate to T4. Agent does not sign.
...
priority:
  ...
  overrides_applied: []
  caps_applied: []
  rationale: "SHOULD on-stage contract pack; MUST at S11 implementation"
...
raised_at: "2026-09-07"
raised_by: "human:front-architect"
source: "Same intake as SUG-20260907-ldc — Figma wireframe extras"
input: >
  Wireframe landing tabs include Recent prospects and ULIP leads; product picker
  includes Savings, ULIP and Health; success screen schedules Online/In-person
  meetings with date, time and meeting link.
...
stage_fit:
  code: SF3
  rationale: >
    ULIP, Savings and Health are later LOB/product-class work. Meeting scheduling is
    absent from the S05 R0 eleven-step blueprint. n+2 from L4 Foundation is premature
    (03 direction rule). Figma is reference only (A11).
  target_stage: "S13 / R1 — expansion after the assisted Term slice"
  unpark_trigger: "R0 completes a real pilot Term sale, or R1 planning starts for those surfaces"

scope:
  code: SC2
  business_scope: "adjacent value — real on the wireframe, nothing in the R0 Term slice fails without it"
  serves: []
  failure_without_it: "no R0 AC fails; SCR-02 is one pipeline; SCR-05 confirms LIFE/TERM only"
  minimal: false
  authority: "BOOT.md WS-3 out_of_scope_now; S11-vertical-slice.md §3; R0-SCOPE A11"

necessity:
  now: NOT-NOW
  future_necessity: MUST
  target_stage: "S13 / R1"
  binds_when: "a second product class or a meeting follow-up is admitted into a journey"
  evidence_tier: E2
  evidence:
    - "BOOT.md out_of_scope_now: ULIP and Savings/Endowment; Health non-life"
    - "S11 §3 explicitly not in the slice: ULIP and savings variants"
    - "S05 §4.1 steps 1-2 have no meeting scheduler"
  confidence: C4
  assumptions: []
  anti_over_engineering:
    X1_named_consumer: false
...
action: PARK
action_rationale: >
  Scope filter SC2 forces PARK to Ideas/scheduled work regardless of future MUST.
  Recorded so the wireframe extras are not re-proposed inside EPIC-003.
conflicts: []
...
priority:
  ...
  rationale: "SF3 + SC2; PRI-5 would be P5 Ideas — kept P4 because it is scheduled R1 work with a named unpark, not an unbounded idea"
...
### SUG-20260903-lif · Life LOB 1SB coverage and adapter standards

```yaml
# schema: triage-record
id: SUG-20260903-lif
raised_at: "2026-09-03"
raised_by: "human:stakeholder"
source: "Cloud agent intake — high-priority stakeholder decision for WS-1"
input: >
  We have one silver bullet integration place in the application. Make sure this
  one silver bullet integration application is working for term life insurance,
  savings, and ULIP — integration of one SB, not just the term — so all APIs cover
  the life line of business. Do not keep all bank-related models in the one silver
  bullet app service; move them to another package. Stop building responses by
  putting command/object values into a Map and serialising the Map; create correct
  JSON payloads with models/JSON objects. Follow right packaging structure,
  SOLID, DRY, design patterns, Java practices; end-to-end review of one silver
  bullet integration. Define guidelines for polling retries / when to stop, and
  ensure resilience including circuit breakers. This is high governance policy /
  high stakeholder decision added into the work stream now as high priority —
  it should not wait or be parked automatically; it has to be admitted with actions.
...
classification:
  type: FUNC
  also: [ARCH, NFR, REFACTOR, DOC, QA]
  breakdown: EPIC
  epic: EPIC-002
  risk_tier: T3
  destination: "PRODUCT-BACKLOG.md E14 · work-items/EPIC-002.work-item.yaml"
...
Recurrence 2026-09-11 (CS-2, not a new row): human restated Term+Savings+ULIP and cited stale adapter findings. Checkout already has FUNC-015 / FUNC-019 / QA-012; LifeLobRegressionIT green. The excluded WS-3 R0 catalogue/journey bag is `SUG-20260911-uls` / `CR-015` (ADMIT-BYPASS transcribed 2026-09-12; HUMAN T4 outstanding) — not a third silent reuse of this row. Remaining adapter gaps: Saving gate-criteria handler, ULIP fund list/performance ports, CR-014 human T4.
