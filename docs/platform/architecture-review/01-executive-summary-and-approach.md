...
3. **Data ownership per service, not shared persistence-for-everything.** (The *physical* half of this — a cluster per service — is withdrawn by `ADR-008`; ownership, credentials and schema separation stand.) Addressed above and in [05](./05-data-architecture.md).
