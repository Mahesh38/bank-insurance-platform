...
| S-20 | BFF → Lead (create / resume / status) | Sync | Client `Idempotency-Key` on create | 2 s | `403 ORIGINATION_RM_ONLY` for any non-`BANK_RM` principal (INV-LED-04). Fail closed. Public path `POST /leads` (`ARCH-023`) |
