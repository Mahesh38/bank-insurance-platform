...
| Service | Release | Wave | Functions / business logic | Representative APIs | Why separate |
|---|---|---|---|---|---|
| **#5 Lead** (Opportunity = durable-demand alias only, `ADR-014`) | **R0 — full origination** | **W1** | **The single origination point (`AC-8`).** Creates the demand object that carries the accountable SP, the `lob` and the origination record; nothing downstream may exist without one | Internal `POST /internal/v1/leads`; BFF `POST /leads` (`ARCH-023`) | Un-deferred from S13 to reconcile with `CURRENT-STATE.yaml` `in_scope` (`AC-9`). Beginning a journey from a customer lookup creates a **second way into the funnel**, which is precisely what `AC-8` forbids. What stays deferred to S13 is campaign and bulk sales-management breadth *on top of* origination — not origination itself |
...
| **Perimeter & Edge Ingress** | **Cloudflare Enterprise (SaaS)** → **F5-XC (SaaS WAF)** → **Amazon API Gateway** (inbound Proxy 1 of 2) → **Internal ALB** (Proxy 2 of 2). No public ALB. Cloudflare and F5 are not AWS and not in any VPC (`ADR-018`). **Apigee is outbound only** (`ADR-020`) | Bank enterprise security standard. Edge TLS termination, L7 policy and rate limiting; no business logic |
...
| **Egress inspection** (`ADR-010` + `ADR-020`) | Inspection VPC per environment, AWS Network Firewall on **pod → Apigee**; **Apigee** is the caller 1SB sees. Do not publish spoke NAT EIPs to 1SB | It is **not a mesh** and does not replace `NetworkPolicy`. The 1SB mTLS session is passed intact, not decrypted. Internal Apigee targets stay private |
