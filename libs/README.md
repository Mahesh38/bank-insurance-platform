...
These are Java 21 / Spring Boot 3.3.x compatible shared libraries for the bank insurance platform.
...
| `ServiceErrorResponse`   | Full RFC7807 response envelope: `type`, `title`, `status`, `detail`, `code`, `retryable`, `upstreamCode?`, `errors[]`. Has factory methods for common cases. |
...
| Class / Type             | Description |
|--------------------------|-------------|
| `BankPrincipal`          | Immutable authenticated actor: `actorId`, `employeeId`, `branchCode`, `roles`, `customerId?`. `toString()` emits only `actorId`. |
...
| Class / Type           | Description |
|------------------------|-------------|
| `AuditEvent`           | Immutable audit record: `eventId`, `timestamp`, `actorId`, `actorType`, `action`, `resourceType`, `resourceId`, `outcome`, `lob`, `journeyId`, `distributorId`, `agentId`, `traceId`, `metadata`. Auto-generates `eventId`/`timestamp` if not provided. |
...
The root `build.gradle.kts` applies the Spring Boot 3.3.5 BOM to all subprojects, so no
explicit Spring/SLF4J versions are needed in individual modules.
