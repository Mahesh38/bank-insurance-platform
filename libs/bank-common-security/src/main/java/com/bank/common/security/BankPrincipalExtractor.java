 ...
 *
 * <p>The consuming service provides the actual JWT validation (e.g. via Spring Security OAuth2
 * Resource Server); this extractor only interprets the already-validated claims.
 *
 * <p>Design rationale: keeping this logic in the shared lib ensures all services parse claims
 * identically. Services wire this into their {@code JwtAuthenticationConverter}.
 */
public final class BankPrincipalExtractor {
  ...
  public static BankPrincipal fromClaims(Map<String, Object> claims) {
    ...
    if (rawRoles instanceof Iterable<?> iterable) {
      roles =
          ((Iterable<String>) iterable).iterator().hasNext()
