...
/**
 * Requires {@code Idempotency-Key} on mutating {@code /v1/**} requests. Same key + same body hash →
 * replay cached response; same key + different body → 409.
 */
@Component
...
public class IdempotencyFilter extends OncePerRequestFilter {
  ...
  public IdempotencyFilter(
      IdempotencyPort idempotencyPort, ObjectMapper objectMapper, ServiceErrors serviceErrors) {
  ...
  protected void doFilterInternal(
    ...
    if (status < 500) {
      idempotencyPort.store(
          key,
          bodyHash,
          new CachedResponse(status, cachingResponse.getContentType(), responseBody, bodyHash));
    }
   ...
   * wording, status and retryability from the catalogue — a caller must not be able to tell whether
   * a refusal came from a filter or a controller.
   */
  private void writeError(
      HttpServletResponse response, int status, String code, String title, String detail)
      throws IOException {
    ServiceErrorResponse body =
        ServiceErrorResponse.of(code)
