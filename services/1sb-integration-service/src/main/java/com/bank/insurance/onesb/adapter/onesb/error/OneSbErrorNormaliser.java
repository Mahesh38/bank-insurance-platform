...
/**
 * Maps 1SB HTTP status + {@code errors[]} bodies to bank {@link ServiceException}s. Controllers
 * never see raw 1SB JSON — only normalised bank error envelopes.
 *
 ...
 * status — into the response the bank caller received (defect D1). The text now goes to {@code
 * reason} on the diagnostic, where support and engineers read it, while {@code detail} comes from
 * {@link com.bank.common.error.ErrorCatalogue}.
 *
  ...
  public ServiceException normalise(int httpStatus, String responseBody) {
    if (httpStatus == 401) {
      return serviceErrors
          .error(ErrorCodes.UPSTREAM_AUTH_FAILURE)
    ...
    if (httpStatus >= 400) {
      ...
      return serviceErrors
          .error(ErrorCodes.UPSTREAM_BUSINESS_ERROR)
          .component("OneSbErrorNormaliser")
  ...
  private ParsedErrors parseErrors(String responseBody) {
    ...
    try {
      ...
      if (!errorsNode.isArray() || errorsNode.isEmpty()) {
        ...
        return new ParsedErrors(message != null ? message : "1SB business error", code, List.of());
      }
      ...
      return new ParsedErrors(
          !detail.isEmpty() ? detail.toString() : "1SB business error", firstUpstreamCode, errors);
    } catch (Exception e) {
      return new ParsedErrors("1SB business error", null, List.of());
    }
  }
