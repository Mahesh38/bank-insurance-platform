...
public class MasterDataService {
  ...
  public MasterLookupOutcome lookup(
      String lob, String lookUpCategory, List<String> entityIds, String manufacturerId) {
    ...
    try {
      MasterLookupResult result =
          masterDataPort.lookup(lob, lookUpCategory, entityIds, manufacturerId);
  ...
  static String cacheKey(
      String lob, String lookUpCategory, String manufacturerId, List<String> entityIds) {
    String sortedIds =
        entityIds.stream().filter(Objects::nonNull).sorted().collect(Collectors.joining(","));
    return lob
        + "|"
        + lookUpCategory
        + "|"
