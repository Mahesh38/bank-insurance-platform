...
public class PaymentService implements PaymentUseCase {
  ...
  private static final Set<JobStatus> PAYABLE_JOB_STATUSES =
      Set.of(JobStatus.COMPLETED, JobStatus.PARTIAL);
  ...
  private void assertPayable(String jobId) {
    ...
    if (!PAYABLE_JOB_STATUSES.contains(job.status())
        || !StringUtils.hasText(job.applicationNumber())) {
  ...
  private ServiceException notPayable(String reason) {
    return serviceErrors
        .error(ErrorCodes.PROPOSAL_NOT_PAYABLE)
        .component("PaymentService")
  ...
  private void publishPaymentUrlRetrieved(
      CreatePaymentCommand command, PaymentSession session, String actorId) {
    try {
      AuditEvent event =
          AuditEvent.builder()
              .actorId(actorId)
