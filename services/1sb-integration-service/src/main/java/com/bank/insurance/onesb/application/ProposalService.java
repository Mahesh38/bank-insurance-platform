...
public class ProposalService implements ProposalUseCase {
  ...
  @Override
  public ProposalSchema getSchema(
      Lob lob, String productCode, String manufacturerId, String version, String quoteJobId) {
    if (lob == null) {
  ...
  public ProposalSubmitResult submit(SubmitProposalCommand command) {
    ...
    if (!StringUtils.hasText(agentId)) {
      throw serviceErrors
          .error(ErrorCodes.AGENT_ATTRIBUTION_MISSING)
          ...
          .errors(
              List.of(
                  ServiceError.ofField(
                      ErrorCodes.AGENT_ATTRIBUTION_MISSING, "agentId is required", "agentId")))
          .build();
    }
    ...
    publishProposalSubmitted(
        command, jobId, actorId, agentId, distributorId, upstream.applicationNumber(), status);
    return new ProposalSubmitResult(jobId, status);
  }

  /**
   * Load proposal job for GET. Unknown id → 404 {@link ErrorCodes#RESOURCE_NOT_FOUND}. Does not
   * fabricate {@code applicationNumber} — returns whatever is stored on the job.
   */
  @Override
  ...
  /**
   * FUNC-025: do not POST a skeleton to 1SB. Empty values fail in the bank. When a schema can be
   * loaded, missing mandatory field names are listed.
   */
  ...
  private ServiceException missingLob(String operation) {
    return serviceErrors
        ...
        .errors(
            List.of(
                ServiceError.ofField(ErrorCodes.MISSING_REQUIRED_FIELD, "lob is required", "lob")))
        .build();
  }

  /**
   * FUNC-004 AC-2 answers 410 here, while catalogue 04 section 6 registers QUOTE_EXPIRED as 409.
   * Both are ratified and they describe different conditions — a quote job that is gone, versus an
   * offer selected past its validity window. The override preserves the approved behaviour and
   * records the disagreement; see 07-PLATFORM-ERROR-CONTRACT.md section 13.
   */
  private ServiceException quoteExpired(String reason) {
    return serviceErrors
        .error(ErrorCodes.QUOTE_EXPIRED)
        .component("ProposalService")
  ...
  private void publishConsentRefMissing(
      SubmitProposalCommand command, String actorId, String agentId, String distributorId) {
    try {
      AuditEvent event =
          AuditEvent.builder()
              .actorId(actorId)
  ...
  private void publishProposalSubmitted(
    ...
    try {
      AuditEvent.AuditEventBuilder builder =
          AuditEvent.builder()
              ...
              .resourceId(jobId)
              .outcome(
                  status == JobStatus.COMPLETED ? AuditOutcomes.SUCCESS : AuditOutcomes.PENDING)
              .lob(command.lob() != null ? command.lob().name() : null)
              .journeyId(command.journeyId())
              .distributorId(distributorId)
              .agentId(agentId)
  ...
  private void publishProposalRejected(
      SubmitProposalCommand command,
      String jobId,
      String actorId,
      String agentId,
