...
/**
 * Quote orchestration (Case 2): validate → create job → LOB handler builds payload → 1SB submit →
 * schedule poll.
 */
@Service
public class QuoteService implements QuoteUseCase {
  ...
  public String createQuote(CreateQuoteCommand command) {
    ...
    String actorId =
        command.actorId() != null && !command.actorId().isBlank() ? command.actorId() : "system";
    String jobId =
        jobStore.createJob(
            command.lob().name(), "QUOTE", command.journeyId(), command.idempotencyKey(), actorId);
  ...
  /**
   * Load quote job for GET. Always returns the stored {@link QuoteJob} (including {@link
   * JobStatus#TIMEOUT}) so the bank can poll status; unknown id → 404 {@link
   * ErrorCodes#RESOURCE_NOT_FOUND}. Does not throw {@link ErrorCodes#QUOTE_TIMEOUT}.
   */
  @Override
