...
/**
 * Application-status orchestration (FUNC-009): 1SB call → normalise → audit. Live call every time —
 * not cached (architecture §7.2), no status-snapshot persistence (see
 * phase-4/FUNC-009-ASSIGNMENT.md for the scope decision).
 */
@Service
public class StatusService implements StatusUseCase {

  private static final Set<String> UW_REQUIREMENTS_STATUSES =
      Set.of(
  ...
  private static final Set<String> UW_IN_PROGRESS_STATUSES =
      Set.of("UNDERWRITING", "SCRUTINY", "PRE_CONVERSION", "ACCEPTED");

  private static final Set<String> CUSTOMER_DECISION_STATUSES =
      Set.of("COUNTER_OFFER", "AWAITING_CLIENT_APPROVAL");

  private static final Set<String> CLOSED_STATUSES =
      Set.of("REJECTED", "CANCELLED", "PROPOSAL_ERROR");
  ...
  public ApplicationStatus getStatus(
      String applicationNumber, Lob lob, String insurerCode, String productCode, String actorId) {
    if (lob == null) {
    ...
    OneSbApplicationStatusResult result =
        upstream.orElseThrow(
            () ->
                serviceErrors
