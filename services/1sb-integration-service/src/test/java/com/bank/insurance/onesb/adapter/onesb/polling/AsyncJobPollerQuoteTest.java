...
class AsyncJobPollerQuoteTest {
  ...
  void pollQuoteUntilDone_pendingThenComplete_withOffers() {
    ...
    when(quotePort.isPollComplete(eq("job-1"), eq("REQ-1"), eq("TERM")))
        .thenAnswer(
            inv -> {
              return calls.incrementAndGet() >= 2;
            });
    when(quotePort.pollQuoteResult("job-1", "REQ-1", "TERM"))
        .thenReturn(
            List.of(
                new QuoteOffer(
                    "o1",
    ...
    List<QuoteOffer> mixed =
        List.of(
            new QuoteOffer(
                ...
                null),
            new QuoteOffer(
                "b", "I2", null, "P", "Bad", null, null, null, false, "ERROR", "declined"));
  ...
  void pollQuoteUntilDone_timeout_failsWithPollTimeout() {
    ...
    verify(jobStore, times(3))
        .recordPollAttempt(eq("job-to"), anyInt(), anyInt(), eq(false), anyInt(), any());
  ...
  void pollQuoteUntilDone_wireMockPendingThenComplete_viaRealAdapter() {
    ...
    LobQuoteHandlerRegistry registry =
        new LobQuoteHandlerRegistry(List.of(handler), TestErrors.ONESB);
    RestClient restClient =
        RestClient.builder().baseUrl(wireMock.baseUrl()).requestFactory(http1Factory()).build();
    OneSbQuoteAdapter realAdapter =
        new OneSbQuoteAdapter(new OneSbHttpClient(restClient), registry, new ObjectMapper());
